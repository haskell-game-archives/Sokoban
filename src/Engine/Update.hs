...
moveBackwards p dir = movePoint p $ fromJust $ lookup dir opposites
  where
    opposites = zip [R,L,U,D] [L,R,D,U]

movePoint :: Point -> Direction -> Point
movePoint p@(x,y) = \case
  R -> (x+1, y)
  L -> (x-1, y)
  U -> (x, y+1)
  D -> (x, y-1)
...
updateBoard dir w = movePlayer dir p' w
  where
    p' = movePoint (getPoint $ player w) dir
...
stepGame f g@(Game i lvl w) =
  if winningGame g
    then goToNextLevelAndSave g
    else return g

stepGameWithoutSave :: Float -> Game -> IO Game
stepGameWithoutSave f g
  | winningGame g = goToNextLevelWithoutSaving g
  | otherwise     = return g
